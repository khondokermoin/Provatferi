...
- **First visit-এ এখন OS পছন্দ অনুসরণ করা হয় (২০২৬-০৯-১৮ থেকে কার্যকর; ১.১-এর light-first default বাতিল)।** অগ্রাধিকার ক্রম একটাই: ব্যবহারকারীর স্পষ্ট পছন্দ (localStorage) > OS pref। যিনি একবার নিজে theme বেছেছেন, তাঁর পছন্দ দুই দিকেই OS-কে হারায় — OS dark হলেও explicit light থাকবে।
- Root-level `prefers-color-scheme` media query এখনও ব্যবহৃত হয় **না**, এবং এটি ইচ্ছাকৃত: resolution একবারই হয়, `<head>`-এর inline script-এ, তাই CSS ও JS দুই জায়গায় দুই রকম সিদ্ধান্ত হওয়ার সুযোগ নেই। globals.css-এর `:root` token গুলো no-JS fallback হিসেবেও কাজ করে।
- Manual light/dark switching বাস্তবায়িত: `ThemeToggle` (sliding day/night switch) `<html>`-এ `data-theme="light"` বা `"dark"` set করে ও `localStorage` key `provatferi-theme`-এ save করে। Switch-এর thumb ও icon-এর অবস্থান সম্পূর্ণভাবে `[data-theme]` থেকে CSS-এ আসে, React state থেকে নয় — তাই hydration-এ thumb লাফ দেয় না।
- `<head>`-এর inline anti-flash script সবসময় একটি নির্দিষ্ট `light`/`dark` মান set করে (attribute ফাঁকা রাখে না), paint-এর আগেই — দুই দিকেই flash of wrong theme এড়ানো হয়।
- Explicit পছন্দ না থাকলে OS পরিবর্তন live follow করা হয় (`matchMedia` listener), reload ছাড়াই। একবার explicit পছন্দ হলে listener আর কিছু বদলায় না।
- Admin panel (`admin.provatferi.org`) হুবহু একই অগ্রাধিকার নিয়ম ও একই motion token ব্যবহার করে; শুধু storage key (`provatferi-admin-theme`) ও control-এর চেহারা (তিন-অপশনের Light/Dark/System dropdown) আলাদা। Motion token দুটি — `--theme-transition-duration: 280ms`, `--theme-transition-ease: cubic-bezier(.4,0,.2,1)` — দুই codebase-এ হাতে মিলিয়ে রাখতে হয়, কারণ তারা stylesheet ভাগ করতে পারে না।
...
| 2026-09-18 | 1.4 | **১.১-এর light-first default বাতিল।** First visit-এ এখন `prefers-color-scheme` অনুসরণ করা হয়; explicit পছন্দ আগের মতোই সর্বোচ্চ অগ্রাধিকার পায় (এখন দুই দিকেই, কারণ script সবসময় একটি নির্দিষ্ট মান লেখে)। কারণ: মালিকের সরাসরি সিদ্ধান্ত (Day/Night Mode Animation task, §10) — নিয়ম ৯.৭ অনুযায়ী নথিভুক্ত। একইসঙ্গে: public site ও admin-এ shared motion token (280ms, `cubic-bezier(.4,0,.2,1)`, `box-shadow` যুক্ত, `prefers-reduced-motion: no-preference`-এ gated), এবং `.theme-toggle` বর্গাকার button-এর বদলে `.theme-switch` sliding day/night switch (thumb + sun/moon cross-fade, `role="switch"`)। Identity palette ও token-এর কোনো মান বদলানো হয়নি। |
